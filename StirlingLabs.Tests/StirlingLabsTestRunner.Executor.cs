...
{
    public const string UriString = "executor://StirlingLabsTest/";

    public static readonly Uri Uri = new(UriString);

    private static long _asmLoadCtxCounter;

    [ThreadStatic]
    private static bool _executingFromTestFrameworkThread;

    public static bool ExecutingFromTestFrameworkThread
    {
        get => _executingFromTestFrameworkThread;
        private set => _executingFromTestFrameworkThread = value;
    }

    private static void NotifyExecutingFromTestFrameworkThread()
        => ExecutingFromTestFrameworkThread = true;

    public CancellationTokenSource CancellationTokenSource = new();

    internal record struct Job
        (Thread Thread, MethodInfo Method, IFrameworkHandle? FrameworkHandle, TestCase TestCase);

    internal record struct JobsByClass
        (Type Class, List<Job> Jobs);
    ...
    public void RunTests(IEnumerable<TestCase>? tests, IRunContext? runContext, IFrameworkHandle? frameworkHandle)
    {
        ...
        var alc = new AssemblyLoadContext("StirlingLabsTestExecutor" + Interlocked.Increment(ref _asmLoadCtxCounter), true);

        var hierarchy = BuildTestCaseHierarchy(tests);

        // <class, <instance, <thread, funcPtr, fw, testCase>>>
        var jobsByClass = new Dictionary<string, JobsByClass>();
        ...
        List<Thread> testThreads = new(CountTests(jobsByClass));

        if (!StartAllTests(jobsByClass, testThreads, ct))
            return;

        frameworkHandle?.SendMessage(TestMessageLevel.Informational, "All tests started.");

        if (!WaitForAllTests(testThreads, ct))
            return;
            ...
            jobsByClass)
    {
        ...
        foreach (var jobs in jobsByClass.Values)
            count += jobs.Jobs.Count;
        return count;

    }
    private static bool WaitForAllTests(
        IEnumerable<Thread> threads, CancellationToken ct)
    {
    ...
    private static bool StartAllTests(
        Dictionary<string, JobsByClass>
            jobsByClass, ICollection<Thread> threads, CancellationToken ct)
    {
    ...
    private static bool InitializeClasses(IFrameworkHandle? frameworkHandle,
        Dictionary<string, JobsByClass>
            jobsByClass, Dictionary<string, Thread> initThreads, CancellationToken ct)
    {
        foreach (var (jobClass, jobs) in jobsByClass.Values)
        {
    ...
    private static bool InitializeAssemblyModules(IFrameworkHandle? frameworkHandle, AssemblyLoadContext alc,
        Dictionary<string, JobsByClass>
            jobsByClass, Dictionary<string, Thread> initThreads, CancellationToken ct)
    {
    ...
    private static void ProcessAssemblies(IFrameworkHandle? frameworkHandle, AssemblyLoadContext alc,
        Dictionary<string, Dictionary<string, List<TestCase>>> hierarchy,
        Dictionary<string, JobsByClass>
            jobsByClass)
    {
...
#pragma warning restore CA1031
    ...
    private static void InitializeAssemblyModule(IFrameworkHandle? frameworkHandle, Assembly asm, string asmName,
        Dictionary<string, Thread> initThreads)
    {
        var thread = new Thread(static o => {
            NotifyExecutingFromTestFrameworkThread();
...
#pragma warning restore CA1031
    ...
    private static void ProcessAssembly(IFrameworkHandle? frameworkHandle, AssemblyLoadContext alc, string asmPath,
        Dictionary<string, JobsByClass>
            jobsByClass, Dictionary<string, List<TestCase>> classGroup)
    {
        ...
        try
        {
            ...
            foreach (var (className, testCases) in classGroup)
            {
                ...
                try
                {
                    ...
                    if (!exists) classJobs = new(classType, new());
                    ...
                    foreach (var testCase in testCases)
                    {
                        try
                        {
                            ...
                            classJobs.Jobs.Add(new(new(runner), method, frameworkHandle, testCase));
                        }
#pragma warning disable CA1031
                        catch (Exception ex)
                        {
